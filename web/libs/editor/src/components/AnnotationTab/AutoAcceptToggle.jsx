...
export const AutoAcceptToggle = injector(
  observer(({ store, annotation, suggestions }) => {
    ...
    // ✅ INIZIA SEMPRE ATTIVO
    useEffect(() => {
      if (!store.autoAcceptSuggestions) {
        store.setAutoAcceptSuggestions(true);
      }
    }, [store.autoAcceptSuggestions]);
          ...
          <Elem name="wrapper" mod={{ loading }}>
            <Space spread>
              {suggestions.size > 0 ? (
                ...
                <Toggle
                  checked={store.autoAcceptSuggestions}  // ✅ USA LO STATO REALE
                  onChange={(e) => {
                    // ✅ NESSUNA CONFERMA - CAMBIO DIRETTO
                    store.setAutoAcceptSuggestions(e.target.checked);
                  }}
                  label="Auto-Accept Suggestions"
                />
              )}
